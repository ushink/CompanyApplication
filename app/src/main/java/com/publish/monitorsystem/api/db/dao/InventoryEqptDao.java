...
public class InventoryEqptDao{
	...
	public boolean isInventoryEqptID(String EquipmentID,String PlanID) {
		boolean flag = false;
		SQLiteDatabase database = null;
		String sql = "select EquipmentID from " + TABLE_NAME + " where PlanID like '%" + PlanID + "%'";
		try {
			database = helper.getReadableDatabase();
			Cursor cursor = database.rawQuery(sql, null);
			while(cursor.moveToNext()){
				String string = cursor.getString(0);
				if(string.equals(EquipmentID)){
					flag = true;
				}
			}
		} catch (Exception e) {
	...
	public List<InventoryEqpt> getAllInventoryEqptList() {
	...
	public List<Eqpt> getInventoryEqptList(String IsInventory,String PlanID) {
		...
		Eqpt eqpt;
		String sql ="select * from eqpt INNER JOIN inventoryeqpt on eqpt.EquipmentID = inventoryeqpt.EquipmentID where inventoryeqpt.IsInventory = "+IsInventory+" and inventoryeqpt.PlanID like '%"+PlanID+"%' order by EquipmentCode;";
		SQLiteDatabase database = null;
		try {
			database = helper.getReadableDatabase();
			Cursor cursor = database.rawQuery(sql, null);
